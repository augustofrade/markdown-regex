export class Transpiler {
    ...
    private closeBlockquote() {
        if(this.isQuotation) {
            ...
            this.linesArray.push("</blockquote>");
        }
    }
    ...
    private parseTable(text: string) {
        if(this.table.header.length === 0) {
            ...
            this.closeTable();
            this.linesArray.push("<p>" + this.parseTags(text) + "</p>");
            this.table.header = [];
        }
    }
    ...
    private closeTable() {
        if(this.table.header.length > 0 && !this.table.open) {
            ...
            this.linesArray.push("<p>" + parsedLine + "</p>");
            ...
            this.linesArray.push("<table>", "\t<tr>", ...tableHeader, "\t</tr>", "</table>");
        ...
        if(this.table.body.length > 0) {
            ...
            this.table.body.forEach(row => {
                this.linesArray.push("\t<tr>");
                row.forEach(cell => {
                    this.linesArray.push("\t\t<td>" + cell + "</td>");
                });
                this.linesArray.push("\t</tr>");
            })
            this.linesArray.push("</table>");
        }
    ...
    private closeList() {
        ...
        if(this.listLevel > 0) {
            this.linesArray.push("\t".repeat(this.listLevel - 1) + "</ul>");
            this.listLevel--;
            this.closeList();
        }
    }
